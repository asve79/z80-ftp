wnd_main
	...
	DB 0
        ...
        DW #0000	;    BUFFER
        DB 0,0		;      LINES
        DB 0		;позиция курсора в окне (от 1)
        DB 22		;нижний ограничитель
        DB 0		; цвет курсора (накладывается по маске из +1(1))
        DB 0		;цвет окна под курсором
	DW txt_m+1	;+16¦   2¦адрес строки для верхнего заголовка окна (если = 0 то игнорируем)
	...
	DB 0
	ENDIF
txt_m	DB 1,'Ftp client v0.1.5',0
