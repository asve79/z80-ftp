...
skiptospace
	...
	IFDEF	WC_PLUGIN
;---------------------------------
;if enter 'get <filename>' command in terminal

ifenterget
	LD	HL, input_bufer
	CALL	skiptospace
	OR	A
	JRNZ	1f			;if filenename not entered
	_fillzero input_bufer,#FF
	_prints msg_specifyfilename
	RET
1	PUSH	HL
	_fillzero filename,#FF
	POP	HL
	INC	HL
	LD	DE,filename
	LD	BC,5
...
;	LDIR
	...
	CALL	getsize
	_cur_on
	_mkfile filestruct		;Create file
	JRZ	6f
	PUSH	AF
	_prints msg_createfile_error	;if error
	POP	AF
	_a_hex
	LD	A,13
	_printc
...
6	_cur_off
	...
	LD	BC,5
	LDIR	
	LD	A,17			;//code 16 - cat request
	CALL	sendandprint
2	 recv	data_descr,data_bufer,#01FF
	OR	A
	JRNZ	4f
	LD	HL,data_bufer
	_cur_off			;---------------------------------------
3	LD	B,1			;//write bufer
	_saveblock512
	JR	2b			;//get next block data
4	LD	A,13
	_printc
	_cur_on
	LD	A,(data_descr)
